...
the life histories, ecologies, and evolutionary histories that drive
each clade's macroevolutionary fitness and characterize its occupancy
...
that family's $\beta_k$ value are negatively correlated
(Fig. \ref{figSupp:betaByRich}). A negative correlation between clade
...
richness should lead to decreasing $\beta_k$ values. Thus we want to
know if this correlation alone accounts for all downstream
superstatistical results. The permutation test is specifically
...
(i.e. equal fitness everywhere), but instead must be rugged. Thus
niche conservatism around local fitness optima in adaptive space
