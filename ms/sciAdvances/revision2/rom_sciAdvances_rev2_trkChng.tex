...
operationally define ``adaptive landscape'' to mean a clade's set \DIFdelbegin \DIFdel{characteristics }\DIFdelend \DIFaddbegin \DIFadd{of
characteristics, and the fitness they impart to the clade, }\DIFaddend that
...
what we term volatilities, of the \DIFdelbegin \DIFdel{stationary distribution of a
homogeneous }\DIFdelend origination-extinction processes of
genera \DIFdelbegin \DIFdel{. Fluctuations
from this stationary process}\DIFdelend \DIFaddbegin \DIFadd{with clades. The details fo this origination-extinction
...
they }\DIFaddend will be approximately Gaussian \DIFdelbegin \DIFdel{if the
...
in different regions of an adaptive landscape. We do not attempt to
...
across an adaptive landscape, this landscape cannot be flat
\DIFaddbegin \DIFadd{(i.e. equal fitness everywhere), but instead must be rugged. Thus
}\DIFaddend niche conservatism around local \DIFaddbegin \DIFadd{fitness }\DIFaddend optima in adaptive space
...
%DIFDELCMD < %%%
\DIFdel{This Poisson distribution is asymptotically Gaussian, which is a more
...
periods considered. The Gaussian }\DIFdelend \DIFaddbegin \DIFadd{The difference of these
Poisson distributions is again asymptotically Gaussian.  Our analysis
does not depend on all clades being perfectly stationary with
$\lambda = \mu$ because of the }\DIFaddend asymptotics of time-averaged \DIFdelbegin \DIFdel{birth-death processeshave been proven and explored elsewhere as well
\mbox{%DIFAUXCMD
\citep{grassmann1987}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Markov
processes. Indeed we zero-center all fluctuation time series to avoid
possible net diversification or extinction bias our analysis of
