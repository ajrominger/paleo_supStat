...
\DIFdelbegin \DIFdel{characteristics }\DIFdelend \DIFaddbegin
\DIFadd{of characteristics, and the fitness they impart to the clade,
}\DIFaddend that influences its macroevolution. Those characteristics
could be ecological (e.g.  substrate preference \citep{bambach1983,
  bush2007, hopkins2014}), morphological (e.g. body plan
\citep{erwin2012}), or macroecological (e.g. range size
...
what we term volatilities, of the \DIFdelbegin \DIFdel{stationary
  distribution of a homogeneous }\DIFdelend origination-extinction
processes of genera \DIFdelbegin \DIFdel{. Fluctuations from this
  stationary process}\DIFdelend \DIFaddbegin \DIFadd{with clades. The
  details of this origination-extinction process, e.g. whether it is
  linear or subject to a carrying capacity, are not of central
  importance to our analysis; so long as fluctuations can reach a
  stationary distribution and are observed over time-averaged
  intervals in a temporally coarse-grained fossil record they
}\DIFaddend will be approximately Gaussian \DIFdelbegin \DIFdel{if the
  clades' diversification dynamics are independent and in local
  equilibrium }\DIFdelend (see Supplemental Section
...
in adaptive space as an emergent trait of a clade \DIFaddbegin \DIFadd{resulting from the
macroevolutionary fitness of the clade and the shape of the
surrounding adaptive landscape}\DIFaddend . Ultimately, volatility emerges from
the life histories, ecologies, and evolutionary histories that \DIFdelbegin \DIFdel{characterize }\DIFdelend \DIFaddbegin \DIFadd{drive
}\DIFaddend each clade's \DIFaddbegin \DIFadd{macroevolutionary fitness and characterize its }\DIFaddend occupancy
of different regions of an adaptive landscape. We do not attempt to
...
matches the \DIFaddbegin \DIFadd{family- and order-level data we
  constructed a 95\% confidence envelope from bootstrapped maximum
  likelihood estimates of $P(x)$. Observed fluctuations for both
  taxonomic levels} \DIFaddend fall within these 95\% confidence
envelopes (Fig. \ref{fig:Px}), indicating that the data do not reject
...
that family's $\beta_k$ value are \DIFaddbegin \DIFadd{negatively }\DIFaddend correlated
(Fig. \ref{figSupp:betaByRich}). \DIFaddbegin \DIFadd{A negative correlation between clade
richness and $\beta_k$ is not unexpected because fluctuations are the
sums of the random variables representing genus origination and
extinction events; the more of these random variables in the summation
(i.e. the more genus richness in a clade) the higher the variance of
the summation.  Because $\beta_k \equiv 1/\sigma_k^2$ increasing
richness should lead to decreasing $\beta_k$ values. }\DIFaddend Thus we want to
know if this correlation alone accounts for all downstream
superstatistical results. \DIFaddbegin \DIFadd{The permutation test is specifically
designed to determine if the $\beta_k$ imposed by this correlation
with richness are sufficient to explain the observed superstatistical
fit.
}\DIFaddend 
...
will be Gamma \citep{cir1985}. \DIFaddbegin \DIFadd{For
  macroevolutionary rates to vary in a way consistent with the
  observed superstatistical description of fluctuations this landscape
  cannot be flat (i.e. equal fitness everywhere), but instead must be
  rugged. Thus }\DIFaddend niche conservatism around local
\DIFaddbegin \DIFadd{fitness }\DIFaddend optima in adaptive space
interrupted by adaptive exploration is likely
