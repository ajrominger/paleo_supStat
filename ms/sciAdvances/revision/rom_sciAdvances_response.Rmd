...
have some questions and comments which I hope are useful. Overall, I
think the manuscript would benefit from:
...
> need to make clear that we're only looking at fluctuations, so everything is 0-centered. Thus when we say equilibrium we mean no wild changes and all coherent w/n an order
...
> The reviewer is indeed correct that net diversification or extinciton could be considered. In our analysis we explicitly did not consider either, and did so by centering all timeseries.  This means we focus on fluctuations about any possible trend toward diversificaiton or extinction.
...
> make clear that we're using standard 10 (or 11?) MY bins used prebiously in the literature
...
> We simpathize with the reviewer for feeling that their valid concerns about sampling were not confronted. Indeed we have not ignored this central statistical issue---we developed our own method to account for differences in sampling (XXXXX). This approach is rooted in the simple idea of binomial sampling that has long been implemented in the paleobiology literature (XXXX).  We take this "three timer correction" (as it is often refered to) and go one step further to correct for sampling differences introduced by publication biase of novel taxa. \newline\newline
The methods developed by Foote and proposed by the reviewer are among the best in the field, and I am a long time admirer of Prof. Foote.  However, these methods were developed for stratographic ranges only---which fail to take full advantage of the richness of occurrence data provided by the PBDB, as noted repeatedly by Alroy (XXXXX)---and cannot account for publication bias.  As such we choose not to empliment them. \newline\newline
The TRiPS method suggested by the reviewer employs a binomial likelihood to quantify sampling. Because the sufficient statistic of a binomial likelihood is simply the observed proportion of successes (or failures) we have indeed mimicked this approach by estimating the observed proportion of "Lazurous" genera (i.e. those that have failed to be deteched between subsequent intervals of positive detection) for a given time period and using this observed proportion to estimate the expected number of taxa actually extant in that time period.  This expected value is simply the number of observed taxa divided by the estimated sampling probability (to confirm this one need only recall that the mean of a binomial distribution is $E[X]$ = np$ thus $n$---the latent number of taxa---is $n = E[X]/p$ assuming the observed number of taxa is on average near the expectation).  The details of our approach are laid out in the methods section and we have added discussion specifically about our decision to develop a new method versus using existing methods.
...
> Agreed, and it was thanks to this reviewer's first round of comments on our manuscript that we changed our title for the better. In our current revision we have XXXXXX

Some other comments.

Figure. 1. There still is no indication of what these orders are.

> We have added their names: XXXXXXX
...
rate, and so: (i) the analysis needs to be re-run with 2018 data. And,
(ii) the text needs to updated to accommodate the steady rate of
...
elsewhere in the paper.  Line 23. I think the authors might mean
"punctuated by innovations that lead to new orders". Line 26. ‘extant’
tends to mean those living today, so maybe say "in the number of taxa
