...
subTaxa <- function(x) {
    ...
    chil <- lapply(as.character(x$taxon_name[tooBig]), function(tname) {
        print(tname)
        out <- pbdb_taxa(name = tname, vocab = 'pbdb', show = 'size', rel = 'children')
...
any(grepl('Canis', foo$taxon_name))



pbdb_taxa(name = 'Nautiloidea', vocab = 'pbdb', show = 'size', rel = 'children')[1, ]
pbdb_taxa(name = 'Nautilida', vocab = 'pbdb', show = 'size', rel = 'children')[1, ]
